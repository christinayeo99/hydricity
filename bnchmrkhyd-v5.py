...
def savedata(PID, sysd, mold, sold):
    ...
    For the plot with all solvents, it can only plot for up to 10 different solvents.
    """
    ...
    for solvent in sold:
        ...
        if i == 0: color = 'salmon'
        if i == 1: color = 'gold'
        if i == 2: color = 'palegreen'
        if i == 3: color = 'cornflowerblue'
        if i == 4: color = 'crimson'
        if i == 5: color = 'olivedrab'
        if i == 6: color = 'darkviolet'
        if i == 7: color = 'aqua'
        if i == 8: color = 'dimgray'
        if i == 9: color = 'orange'
        
        axall.scatter(listdict[calOR], listdict[expOR], c=color, label=solname)
        axall.scatter(listdict[calOM], listdict[expOM], c=color)
        
        i += 1

        #Combine data to single list
        allcal += listdict[calOR] + listdict[calOM]
        allexp += listdict[expOR] + listdict[expOM]
