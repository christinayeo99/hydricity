...
def savedata(PID, sysd, mold, sold):
    ...
    for YID in sysd:
        ...
        if mold[MID]['type'] == 'OR':
            if sold[SID]['sname'] == 'acetonitrile':
                cal_aceto_OR.append(hydricity)
                exp_aceto_OR.append(float(sysd[YID]['hyd']))
            if sold[SID]['sname'] == 'benzonitrile':
                cal_benzo_OR.append(hydricity)
                exp_benzo_OR.append(float(sysd[YID]['hyd']))
            if sold[SID]['sname'] == 'DMSO':
                cal_DMSO_OR.append(hydricity)
                exp_DMSO_OR.append(float(sysd[YID]['hyd']))
            if sold[SID]['sname'] == 'water':
                cal_water_OR.append(hydricity)
                exp_water_OR.append(float(sysd[YID]['hyd']))
        else:
            if sold[SID]['sname'] == 'acetonitrile':
                cal_aceto_OM.append(hydricity)
                exp_aceto_OM.append(float(sysd[YID]['hyd']))
            if sold[SID]['sname'] == 'benzonitrile':
                cal_benzo_OM.append(hydricity)
                exp_benzo_OM.append(float(sysd[YID]['hyd']))
            if sold[SID]['sname'] == 'DMSO':
                cal_DMSO_OM.append(hydricity)
                exp_DMSO_OM.append(float(sysd[YID]['hyd']))
            if sold[SID]['sname'] == 'water':
                cal_water_OM.append(hydricity)
                exp_water_OM.append(float(sysd[YID]['hyd']))      
    ...
    axall.scatter(cal_aceto_OR, exp_aceto_OR, c='salmon', label = 'Acetonitrile')
    axall.scatter(cal_aceto_OM, exp_aceto_OM, c='salmon')
    axall.scatter(cal_benzo_OR, exp_benzo_OR, c='gold', label = 'Benzonitrile')
    axall.scatter(cal_benzo_OM, exp_benzo_OM, c='gold')
    axall.scatter(cal_DMSO_OR, exp_DMSO_OR, c='palegreen', label = 'DMSO')
    axall.scatter(cal_DMSO_OM, exp_DMSO_OM, c='palegreen')
    axall.scatter(cal_water_OR, exp_water_OR, c='cornflowerblue', label = 'Water')
    axall.scatter(cal_water_OM, exp_water_OM, c='cornflowerblue')
    #Combine data to single list
    allcal = cal_aceto_OR + cal_aceto_OM + cal_benzo_OR + cal_benzo_OM + cal_DMSO_OR + cal_DMSO_OM + cal_water_OR + cal_water_OM
    allexp = exp_aceto_OR + exp_aceto_OM + exp_benzo_OR + exp_benzo_OM + exp_DMSO_OR + exp_DMSO_OM + exp_water_OR + exp_water_OM
    ...
    axall.legend()
    #Save plot to pdf
    figall.savefig('%s_all.pdf' % PID)
    
    #Acetonitrile
    #Make scatter plot
    figaceto, axaceto = plt.subplots()
    axaceto.scatter(cal_aceto_OR, exp_aceto_OR, c='turquoise', label = "Organic")
    axaceto.scatter(cal_aceto_OM, exp_aceto_OM, c='darkgray', label = "Organometallic")
    #Combine data to single list
    acetocal = cal_aceto_OR + cal_aceto_OM
    acetoexp = exp_aceto_OR + exp_aceto_OM
    #Add trendline and calculate R^2
    z = np.polyfit(acetocal, acetoexp, 1)
    p = np.poly1d(z)
    axaceto.plot(acetocal,p(acetocal),"k")
    correlation_matrix = np.corrcoef(acetocal, acetoexp)
    correlation_xy = correlation_matrix[0,1]
    r_squared = correlation_xy**2
    #Add labels and legend
    axaceto.set_title('Acetonitrile\ny = %f x + %f, R^2 = %f' % (z[0], z[1], r_squared))
    axaceto.set_xlabel('Calculated hydricity (kcal/mol)')
    axaceto.set_ylabel('Experimental Hydricity (kcal/mol)')
    axaceto.legend()
    #Save plot to pdf
    figaceto.savefig('%s_aceto.pdf' % PID)
    
    #Benzonitrile
    #Make scatter plot
    figbenzo, axbenzo = plt.subplots()
    axbenzo.scatter(cal_benzo_OR, exp_benzo_OR, c='turquoise', label = "Organic")
    axbenzo.scatter(cal_benzo_OM, exp_benzo_OM, c='darkgray', label = "Organometallic")
    #Combine data to single list
    benzocal = cal_benzo_OR + cal_benzo_OM
    benzoexp = exp_benzo_OR + exp_benzo_OM
    #Add trendline and calculate R^2
    z = np.polyfit(benzocal, benzoexp, 1)
    p = np.poly1d(z)
    axbenzo.plot(benzocal,p(benzocal),"k")
    correlation_matrix = np.corrcoef(benzocal, benzoexp)
    correlation_xy = correlation_matrix[0,1]
    r_squared = correlation_xy**2
    #Add labels and legend
    axbenzo.set_title('Benzonitrile\ny = %f x + %f, R^2 = %f' % (z[0], z[1], r_squared))
    axbenzo.set_xlabel('Calculated hydricity (kcal/mol)')
    axbenzo.set_ylabel('Experimental Hydricity (kcal/mol)')
    axbenzo.legend()
    #Save plot to pdf
    figbenzo.savefig('%s_benzo.pdf' % PID)
    
    #DMSO
    #Make scatter plot
    figDMSO, axDMSO = plt.subplots()
    axDMSO.scatter(cal_DMSO_OR, exp_DMSO_OR, c='turquoise', label = "Organic")
    axDMSO.scatter(cal_DMSO_OM, exp_DMSO_OM, c='darkgray', label = "Organometallic")
    #Combine data to single list
    DMSOcal = cal_DMSO_OR + cal_DMSO_OM
    DMSOexp = exp_DMSO_OR + exp_DMSO_OM
    #Add trendline and calculate R^2
    z = np.polyfit(DMSOcal, DMSOexp, 1)
    p = np.poly1d(z)
    axDMSO.plot(DMSOcal,p(DMSOcal),"k")
    correlation_matrix = np.corrcoef(DMSOcal, DMSOexp)
    correlation_xy = correlation_matrix[0,1]
    r_squared = correlation_xy**2
    #Add labels and legend
    axDMSO.set_title('DMSO\ny = %f x + %f, R^2 = %f' % (z[0], z[1], r_squared))
    axDMSO.set_xlabel('Calculated hydricity (kcal/mol)')
    axDMSO.set_ylabel('Experimental Hydricity (kcal/mol)')
    axDMSO.legend()
    #Save plot to pdf
    figDMSO.savefig('%s_DMSO.pdf' % PID)
    
    #Water
    #Make scatter plot
    figwater, axwater = plt.subplots()
    axwater.scatter(cal_water_OR, exp_water_OR, c='turquoise', label = "Organic")
    axwater.scatter(cal_water_OM, exp_water_OM, c='darkgray', label = "Organometallic")
    #Combine data to single list
    watercal = cal_water_OR + cal_water_OM
    waterexp = exp_water_OR + exp_water_OM
    #Add trendline and calculate R^2
    z = np.polyfit(watercal, waterexp, 1)
    p = np.poly1d(z)
    axwater.plot(watercal,p(watercal),"k")
    correlation_matrix = np.corrcoef(watercal, waterexp)
    correlation_xy = correlation_matrix[0,1]
    r_squared = correlation_xy**2
    #Add labels and legend
    axwater.set_title('water\ny = %f x + %f, R^2 = %f' % (z[0], z[1], r_squared))
    axwater.set_xlabel('Calculated hydricity (kcal/mol)')
    axwater.set_ylabel('Experimental Hydricity (kcal/mol)')
    axwater.legend()
    #Save plot to pdf
    figwater.savefig('%s_water.pdf' % PID)
